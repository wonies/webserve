...
void CGI::_assignScriptBin(void) {
...
void CGI::_assignEnvironList(void) {
...
void CGI::proceed(const Request& rqst, process_t& procs) {
  log("CGI\t: proceed");
...
void CGI::_detach(const Request& rqst, process_t& procs) {
...
/* PARENT */
void CGI::writeTo(const process_t& procs, const char* in_body,
                  const size_t& size) {
  if (write(procs.fd[W], in_body, size) == ERROR) throwSysErr("write", 500);
}

void CGI::wait(process_t& procs) {
...
void CGI::build(msg_buffer_t& out) {
  _buildLine(out);
...
void CGI::_buildLine(msg_buffer_t& out) {
  out.msg << HTTP::http.signature << '/' << HTTP::http.version.at(VERSION_11)
          << SP;

  if (!streamsize(out.body))
    out.msg << "204" << SP << HTTP::key.status.at(204) << CRLF;
  else
    out.msg << "200" << SP << HTTP::key.status.at(200);

  out.msg << CRLF;
}

void CGI::_buildHeader(msg_buffer_t& out) {
...
void CGI::_buildHeaderServer(msg_buffer_t& out) {
  // Add server info
  out.msg << HTTP::key.header_out.at(OUT_SERVER) << ':' << SP << software
          << CRLF;

  // Add date
  out.msg << HTTP::key.header_out.at(OUT_DATE) << ':' << SP
          << timeToStr(getNow()) << CRLF;

  // Add connection
  out.msg << HTTP::key.header_out.at(OUT_CONNECTION) << ':' << SP
          << HTTP::http.connection.at(CN_KEEP_ALIVE) << CRLF;
}

void CGI::_buildHeaderType(msg_buffer_t& out, const size_t& pos_header_end) {
  if (!found(pos_header_end) ||
      (found(pos_header_end) &&
       !found(out.msg.str().find(HTTP::key.header_out.at(OUT_CONTENT_TYPE))))) {
    out.msg << HTTP::key.header_out.at(OUT_CONTENT_TYPE) << ':' << SP
            << HTTP::key.mime.at("txt") << CRLF;
  }
}

void CGI::_buildHeaderLen(msg_buffer_t& out, const size_t& pos_header_end) {
  if (!found(pos_header_end) ||
      (found(pos_header_end) &&
       !found(out.msg.str().find(HTTP::key.header_out.at(OUT_CONTENT_LEN))))) {
    out.msg << HTTP::key.header_out.at(OUT_TRANSFER_ENC) << ':' << SP
...
void CGI::_buildChunk(msg_buffer_t& out) {
  sstream_t chunked;
...
/* CHILD */
void CGI::_buildEnviron(const Request& rqst, process_t& procs) {
  for (uint_t idx = 0; _buildEnvironVar(rqst, procs, idx); ++idx);
}

bool CGI::_buildEnvironVar(const Request& rqst, process_t& procs, uint_t idx) {
  try {
    osstream_t oss;
    ...
    switch (idx) {
      case SERVER_NAME:
        oss << rqst.header().host;
        break;
      case SERVER_PORT:
        oss << rqst.config().listen;
        break;
      case SERVER_PROTOCOL:
        oss << HTTP::http.signature << '/' << HTTP::http.version.at(VERSION_11);
        break;
      case REMOTE_ADDR:
        break;
      case REMOTE_HOST:
      ...
      case GATEWAY_INTERFACE:
        break;
      case REQUEST_METHOD:
        oss << str_method[rqst.line().method];
        break;
      case SCRIPT_NAME:
        oss << rqst.line().uri.substr(rqst.line().uri.rfind('/') + 1);
        break;
      ...
      case CONTENT_TYPE:
        if (rqst.line().method == POST) oss << rqst.header().content_type;
        break;

      case PATH_INFO:
        oss << rqst.line().uri.substr(rqst.location().root.length() + 1);
        break;
      ...
      case QUERY_STRING:
        ...
        break;
      case UPLOAD_DIR:
        oss << rqst.location().upload;
        break;
      case HTTP_COOKIE:
        oss << rqst.header().cookie;
        break;
    }
...
bool CGI::_redirect(const process_t& procs) {
...
void CGI::_execve(const process_t& procs) {
  vec_cstr_t argv_c;
  vec_cstr_t env_c;
...
void CGI::_assignVectorChar(vec_cstr_t& vec_char, const vec_str_t& vec_str) {
  for (vec_str_t::const_iterator iter = vec_str.begin(); iter != vec_str.end();
       ++iter)
