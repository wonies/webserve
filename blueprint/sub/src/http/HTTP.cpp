 ...
 */
size_t HTTP::setConfig(const str_t& host, const vec_config_t& configs) {
...
bool HTTP::_setConfigMatchName(const str_t& host, const vec_str_t& names,
                               const uint_t& listen) {
  isstream_t iss(host);
  ...
  try {
    ...
    // std::clog << "name: " << name << ", port: " << port << ", found_name: "
    // << distance( names, name ) << std::endl;
    return distance(names, name) != NOT_FOUND && port == listen;
  } catch (err_t& err) {
...
size_t HTTP::setLocation(const path_t& uri, const vec_location_t& locations) {
  if (locations.size() > 1) {
    size_t dot = uri.find('.');
    ...
    idx = 1;
    for (iter = locations.begin() + 1; iter != locations.end(); ++iter) {
      if (uri.find(iter->path) == 0 && iter->path.length() > precise)
        result = idx;
      ++idx;
    }
    if (result != NOT_FOUND) return result;
