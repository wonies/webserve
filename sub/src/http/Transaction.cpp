...
connection_e Transaction::connection(void) {
  if (_rqst.header().connection == CN_CLOSE ||
      _rspn.header().connection == CN_CLOSE)
    return CN_CLOSE;
  else
    return CN_KEEP_ALIVE;
}
...
Transaction::Transaction(Client& client) : _cl(client), _rqst(client) {
  ...
  if (!getInfo(_rqst.line().uri, _rqst.info) &&
      _rqst.location().rewrite.empty()) {
    if (errno == 2) throw errstat_t(404, err_msg[SOURCE_NOT_FOUND]);
    if (errno == 20)
      throw errstat_t(404, err_msg[SOURCE_NOT_DIR]);
    else
      throw errstat_t(500);
  }
...
void Transaction::act(void) {
  _rspn.act(_rqst);
  build(_rspn, _cl.out);
}

void Transaction::_setBodyEnd(void) {
...
bool Transaction::_invokeCGI(const Request& rqst, process_t& procs) {
  if (!rqst.location().cgi || isDir(rqst.info) || !isExist(rqst.line().uri))
    return FALSE;
  ...
  if (ext != "cgi" && ext != "exe") {
    try {
      procs.argv.push_back(CGI::script_bin.at(ext));
...
bool Transaction::recvMsg(msg_buffer_t& in, const char* buf,
                          ssize_t& byte_read) {
  if (!in.msg_done) {
    in.msg.write(buf, byte_read);

    size_t pos_header_end = in.msg.str().find(
        MSG_END, in.msg_read - (in.msg_read * (in.msg_read < 3) +
                                3 * !(in.msg_read < 3)));
    ...
    if (!found(pos_header_end))
      ...
      if (in.body_read) in.body.write(&buf[body_begin], in.body_read);
...
bool Transaction::recvBody(msg_buffer_t& in, const process_t& procs,
                           const char* buf, const ssize_t& byte_read) {
  // Keep FALSE untill meet the content-length or tail of chunk (0CRLFCRLF)
  if (!in.chunk)
    return _recvBodyPlain(in, procs, buf, byte_read);
  else {
...
bool Transaction::_recvBodyPlain(msg_buffer_t& in, const process_t& procs,
                                 const char* buf, const ssize_t& byte_read) {
  ...
  if (!procs.pid)
    ...
    std::clog << "sending body------------" << std::endl;
  ...
  if (in.body_read) {
    ...
    oss << "TCP\t: body read by " << byte_read << " so far: " << in.body_read
        << " / " << in.body_size << std::endl;
    log(oss.str());
  }
...
bool Transaction::_recvBodyChunk(msg_buffer_t& in, const process_t& procs,
                                 const char* buf) {
  isstream_t iss(buf);
...
bool Transaction::_recvBodyChunkData(msg_buffer_t& in, const process_t& procs,
                                     isstream_t& iss) {
  char data[SIZE_BUF];
  ...
  while (frac) {
    ...
    iss.read(data, frac);
    if (!procs.pid)
      in.body.write(data, frac);
    else if (!dead(procs))
      CGI::writeTo(procs, data, frac);
    else
      return TRUE;
...
bool Transaction::_recvBodyChunkPredata(msg_buffer_t& in,
...
bool Transaction::_recvBodyChunkIncomplete(msg_buffer_t& in,
                                           const process_t& procs,
                                           isstream_t& iss) {
  char data[SIZE_BUF];
  ...
  if (!procs.pid)
    in.body.write(data, in.incomplete);
  else if (!dead(procs))
    CGI::writeTo(procs, data, in.incomplete);
  else
    return TRUE;
...
void Transaction::build(const Response& rspn, msg_buffer_t& out) {
  _buildLine(rspn, out.msg);
  _buildHeader(rspn, out.msg);
  if (rspn.body()) _buildBody(rspn, out);
}

void Transaction::_buildLine(const Response& rspn, sstream_t& out_msg) {
  map_uint_str_t::iterator iter = HTTP::key.status.find(rspn.line().status);

  out_msg << HTTP::http.signature << '/'
          << HTTP::http.version.at(static_cast<size_t>(rspn.line().version))
          << SP << iter->first << SP << iter->second << CRLF;
}

void Transaction::_buildHeader(const Response& rspn, sstream_t& out_msg) {
...
void Transaction::_buildHeaderName(uint_t id, sstream_t& out_msg) {
  out_msg << HTTP::key.header_out.at(id) << ": ";
}

void Transaction::_buildHeaderValue(const response_header_t& header, uint_t id,
                                    sstream_t& out_msg) {
  switch (id) {
    case OUT_SERVER:
      out_msg << header.server;
      break;
    case OUT_DATE:
      out_msg << timeToStr(header.date);
      break;
    case OUT_CONNECTION:
      out_msg << str_connection[header.connection];
      break;
    case OUT_TRANSFER_ENC:
      out_msg << HTTP::http.encoding.at(header.transfer_encoding);
      break;
    case OUT_CONTENT_LEN:
      out_msg << header.content_length;
      break;
    case OUT_CONTENT_TYPE:
      out_msg << header.content_type;
      break;
    case OUT_LOCATION:
      out_msg << header.location;
      break;
    ...
    case OUT_SET_COOKIE:
      out_msg << header.cookie;
      break;
  }
  out_msg << CRLF;
}

void Transaction::_buildBody(const Response& rspn, msg_buffer_t& out) {
  out.body << rspn.body().rdbuf();
}

void Transaction::buildError(const uint_t& status, Client& cl) {
  if (cl.action)
    build(Response(status, cl.action->config()), cl.out);
  else
    build(Response(status, cl.server().configDefault()), cl.out);
}
